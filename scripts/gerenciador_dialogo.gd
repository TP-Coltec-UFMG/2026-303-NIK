		...
		if dados_dialogos != null:
			...
			for dialogo in dados_dialogos:
				...
				for escolha in dialogo.escolhas:
					...
					necessarias.assign(escolha.tags_necessarias)
					
					var proibidas : Array[String] = []
					proibidas.assign(escolha.tags_proibidas)
					
					var adicionadas : Array[String] = []
					adicionadas.assign(escolha.tags_adicionadas)

					escolhas.append(EscolhaDialogo.new(escolha.texto, necessarias, proibidas, adicionadas, escolha.proxima_arvore))
